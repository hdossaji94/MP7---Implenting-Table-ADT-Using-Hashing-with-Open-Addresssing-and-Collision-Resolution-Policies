...
int main(int argc, char **argv)
{
    ...
    int *ip = 0;
    table_t *H;
    //unitdriver that tests basic insertions and deletions
    if(UnitDriver == 0){
            H = table_construct(15, ProbeDec);
            hashkey_t startkey = MINID + (15 - MINID%15);
             for (i = 0; i < 15-1; i++) {
                table_insert(H, startkey+i, ip);
             }
        table_debug_print(H);
        table_delete(H, 19);
        table_delete(H, 19);
        ...
        table_delete(H, 24);
        table_debug_print(H);
        table_insert(H, 15, ip);
        table_insert(H, 32, ip);
        table_insert(H, 53, ip);
        table_debug_print(H);
        table_insert(H, 432, ip);
        table_debug_print(H);
                    
    }
    //unitdriver that tests boundaries of hash table
    if(UnitDriver == 1){
        H = table_construct(8, ProbeDec);
        table_insert(H, 15, ip);
        table_insert(H, 32, ip);
        table_insert(H, 53, ip);
        table_insert(H, 52, ip);
        table_insert(H, 4, ip);
        table_debug_print(H);
        table_delete(H, 15);
        table_delete(H, 32);
    ...
    //h(K) = K %7 and linear probing. simple insertions and deletions and printing
    if(DeletionDriver == 1){
         H = table_construct(7, ProbeDec);
    ...
    //h(k) = k%7 and linear probing with table becoming full with simple insertions deletions and printing
    if(DeletionDriver == 2){
         H = table_construct(7, ProbeDec);
    ...
    if(UnitDriver == 0 || UnitDriver == 1 || DeletionDriver == 1 || DeletionDriver == 2 )
    table_destruct(H);
    return 0;
}
...
void getCommandLine(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "d:u:m:a:h:i:t:s:erbv")) != -1)
        switch(c) {
            case 'd': DeletionDriver = atoi(optarg); break;
